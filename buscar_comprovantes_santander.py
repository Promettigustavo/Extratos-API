    ...
    def baixar_pdf(self, url_download: str, payment_id: str, save_path: Path = None) -> str:
        ...
        try:
            ...
            if save_path is None:
                ...
                # Nome do arquivo no formato: comprovante-ibe-{UUID}.pdf
                # O payment_id já vem como UUID da API
                payment_id_safe = payment_id.replace('/', '-').replace('\\', '-').replace(':', '-')
                save_path = dir_data / f"comprovante-ibe-{payment_id_safe}.pdf"
